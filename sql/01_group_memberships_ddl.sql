...
CREATE TABLE IF NOT EXISTS `private_dataset.group_memberships` (
  user_email STRING NOT NULL OPTIONS(description="Lowercase email address of the user"),
  group_email STRING NOT NULL OPTIONS(description="Lowercase email address of the Cloud Identity group")
)
CLUSTER BY user_email, group_email
OPTIONS(
  description="Transitive group memberships synced from Google Cloud Identity for CLS/RLS enforcement"
);
